...
function updateImages(result) {
  ...
  for(var range = 0; range < result.valueRanges.length; range++) {
      for(var row=0; row<result.valueRanges[range].values.length; row++, i++) {
        // check if queryString Memberid matches the first field in the row
        if (queryString("memberId") != null && result.valueRanges[range].values[row][1]==queryString("memberId")) {
            g++
            // check if queryString Memberid matches the first field in the row and update grafic link
            document.getElementById(g+":6-img").src = "https://chart.googleapis.com/chart?chs=150x150&cht=qr&chl="+encodeURIComponent(result.valueRanges[range].values[g][4]);
            document.getElementById(g+":6-a").href = "https://chart.googleapis.com/chart?chs=150x150&cht=qr&chl="+encodeURIComponent(result.valueRanges[range].values[g][4]);
        }
      }  
  }
}
